...
namespace System.Management.Classes
{
	...
	internal class UNIX_Process : CIM_Process
	{
		...
		public override UNIX_MethodParameterClass Create (UNIX_MethodParameterClass inProperties)
		{
			string cmd = inProperties.GetPropertyAs<string>("CommandLine");
			string wd = inProperties.GetPropertyAs<string>("WorkingDirectory");
			cmd = "-c \"" + cmd + "\"";
			System.Diagnostics.ProcessStartInfo startInfo = new System.Diagnostics.ProcessStartInfo("bash", cmd);
			...
			var result = base.Create (inProperties);
			result.RegisterProperty ("ProcessId", CimType.SInt32, 0);
			result.AddProperty ("ProcessId", SafeGetProcessId (p));
			return result;
		}

		public override UNIX_MethodParameterClass Terminate (UNIX_MethodParameterClass inProperties)
		{
			var id = ProcessId;
			if (id > 0)
			{
			...
			var result = base.Terminate (inProperties);
			result.RegisterProperty ("Reason", CimType.SInt32, 0);
			result.RegisterProperty ("Reason", CimType.String, 0);
			result.AddProperty ("Reason", 0);
			result.AddProperty ("Message", string.Format("Process Id {0} has been terminated", id));
			return result;
		}
